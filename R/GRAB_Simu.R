## ------------------------------------------------------------------------------
## GRAB_Simu.R
## Simulation utilities for synthetic genotype/haplotype data, pedigrees,
## and phenotypes (binary, survival, quantitative). Provides helpers to
## construct families, generate SNP/haplotype matrices, create PLINK files,
## and simulate trait vectors under user-specified genetic architectures.
##
## Functions (primary):
##   GRAB.SimuGMat              : Simulate genotype matrix (families + unrelated)
##   GRAB.makePlink             : Write genotype matrix to PLINK bed/bim/fam trio
##   GRAB.SimuGMatFromGenoFile  : Derive simulated sets from existing PLINK data
##   GRAB.SimubVec              : Simulate linear predictor components (b vector)
##   GRAB.SimuPheno             : Generate phenotypes (binary / survival / quantitative)
##
## Internal helpers:
##   checkInput, example.fam.*  : Validate design & build family pedigrees
##   make.SNP.info, geno.simu   : SNP info table and unrelated genotypes
##   haplo.simu, from.haplo.to.geno, from.geno.to.haplo : Haplotype <-> genotype ops
##   f.binary, f.surv, getProb, getEps : Trait-specific probability / noise generators
## ------------------------------------------------------------------------------
