                           ...
                           std::vector<std::string> & t_SampleInBgen)
{
  ...
  uint32_t offset; 
  if(fread(&offset, 4, 1, m_fin) != 1) { /* error */ }
  uint32_t L_H; 
  if(fread(&L_H, 4, 1, m_fin) != 1) { /* error */ }
  if(fread(&m_M0, 4, 1, m_fin) != 1) { /* error */ }
  std::cout << "snpBlocks (Mbgen): " << m_M0 << std::endl;
  assert(m_M0 != 0);
  
  if(fread(&m_N0, 4, 1, m_fin) != 1) { /* error */ }
  ...
  char magic[5]; 
  if(fread(magic, 1, 4, m_fin) != 4) { /* error */ }
  magic[4] = '\0'; 
  fseek(m_fin, L_H-20, SEEK_CUR); 
  uint32_t flags; 
  if(fread(&flags, 4, 1, m_fin) != 1) { /* error */ }
                                  ...
                                  bool& t_isBoolRead)        // only used in BGEN, Wei, if you want, you can add details here.
{
  ...
  if ( numBoolRead > 0 ) {
    ...
    uint32_t physpos; 
    if(fread(&physpos, 4, 1, m_fin) != 1) { /* handle error */ }
    position = physpos;
    
    uint16_t K; 
    if(fread(&K, 2, 1, m_fin) != 1) { /* handle error */ } 
    if (K != 2) {
    ...
    uint32_t LA; 
    if(fread(&LA, 4, 1, m_fin) != 1) { /* handle error */ }
    if (LA > maxLA) {
      maxLA = 2*LA;
      free(allele1);
      allele1 = (char *) malloc(maxLA+1);
    }
    if(fread(allele1, 1, LA, m_fin) != LA) { /* handle error */ }
    allele1[LA] = '\0';
    first_allele = std::string(allele1);
    free(allele1);
    
    uint32_t LB; 
    if(fread(&LB, 4, 1, m_fin) != 1) { /* handle error */ }
    if (LB > maxLB) {
      maxLB = 2*LB;
      free(allele0);
      allele0 = (char *) malloc(maxLB+1);
    ...
    if(fread(allele0, 1, LB, m_fin) != LB) { /* handle error */ }
    allele0[LB] = '\0';
    second_allele = std::string(allele0);
    free(allele0);
    
    uint32_t C; 
    if(fread(&C, 4, 1, m_fin) != 1) { /* handle error */ }
    if (C > m_zBuf.size()) m_zBuf.resize(C-4);
    
    uint32_t D; 
    if(fread(&D, 4, 1, m_fin) != 1) { /* handle error */ }
    m_zBufLens = C-4; m_bufLens = D;
    
    if(fread(&m_zBuf[0], 1, C-4, m_fin) != (size_t)(C-4)) { /* handle error */ }
