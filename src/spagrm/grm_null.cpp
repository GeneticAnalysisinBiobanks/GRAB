...
SPAGRMClass buildSPAGRMNullModel(const Eigen::VectorXd &Resid,
    ...
    infoMsg("  MAF interval grid (%zu bins): [%.2g .. %.2g]", mafInterval.size(), mafInterval.front(),
            mafInterval.back());
    ...
    auto familyWorker = [&](int tid) {
        ...
        while (true) {
            ...
            for (const auto &e : familyEntries[fi]) {
                ...
                if (e.row < e.col)
                    famEdges.push_back(
                        {g2l[e.row], g2l[e.col], e.value, std::abs(e.value * Resid[e.row] * Resid[e.col])});
            }
            std::sort(famEdges.begin(), famEdges.end(),
                      [](const OffDiagEntry &a, const OffDiagEntry &b) { return a.cov < b.cov; });
            ...
            std::vector<bool> edgeKept(nEdges, false);
            {
                ...
                for (int k = removeUpTo; k >= 0; --k) {
                    ...
                    if (ra == rb) {
                        edgeKept[k] = true;
                    } else if (uf.componentSize(ra) + uf.componentSize(rb) <= static_cast<uint32_t>(MAX_NUM_IN_FAM)) {
                        uf.unite(ra, rb);
                        edgeKept[k] = true;
                    }
                }
    ...
    infoMsg("  %zu singletons, %zu families (%zu needed splitting)", singletonSet.size(), families.size(),
            nSplitFamilies);
    ...
    infoMsg("  Built %zu two-subject, %zu multi-subject family models, "
            "%zu unrelated outliers",
            twoSubj_resid_list.size(), threeSubj_CLT_list.size(), unrelatedOutlierResids.size());

    Eigen::VectorXd residOutliers = Eigen::Map<Eigen::VectorXd>(
        unrelatedOutlierResids.data(), static_cast<Eigen::Index>(unrelatedOutlierResids.size()));
    ...
    return SPAGRMClass(Resid, sum_R_nonOutlier, R_GRM_R_nonOutlier, R_GRM_R_TwoSubjOutlier, R_GRM_R, mafInterval,
                       std::move(fd), spaCutoff, ZETA_DEFAULT, TOL_DEFAULT);
}
