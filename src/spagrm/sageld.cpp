...
std::vector<EnvSpec> parseEnvSpecs(int nRC, const std::vector<std::string>& colNames)
{
  if (nRC < 3 || (nRC - 1) % 2 != 0)
    throw std::runtime_error(
        "SAGELD residual file: expected odd number of columns ≥ 3 "
        "(R_G  R_<E1>  R_Gx<E1>  [R_<E2>  R_Gx<E2>  ...]), "
        "got " + std::to_string(nRC));
  ...
  if (static_cast<int>(colNames.size()) != nRC)
    throw std::runtime_error(
        "SAGELD: column name count (" + std::to_string(colNames.size()) +
        ") does not match data column count (" + std::to_string(nRC) + ")");

  if (colNames[0] != "R_G")
    throw std::runtime_error(
        "SAGELD: first residual column must be 'R_G', got '" + colNames[0] + "'");
  ...
  for (int i = 0; i < nEnv; ++i) {
    ...
    if (envFromE != envFromGxE)
      infoMsg("  Warning: env name mismatch '%s' vs '%s' in column pair %d/%d",
              eName.c_str(), gxName.c_str(), ce, cg);
...
public:
  struct PerEnv { SPAGRMClass spagrm_combined; };
  ...
  std::string getHeaderColumns() const override {
    std::string h = "\tP_G";
    for (const auto& n : m_envNames) h += "\tP_Gx" + n;
    h += "\tZ_G";
    for (const auto& n : m_envNames) h += "\tZ_Gx" + n;
    return h;
  }
...
void runSAGELD(
  ...
  infoMsg("Starting SAGELD marker-level association (%d threads, %d env(s))...",
          nthreads, nEnv);
