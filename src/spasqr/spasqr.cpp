...
class SPAsqrMethod : public MethodBase {
  public:
    ...
    std::unique_ptr<MethodBase> clone() const override { return std::make_unique<SPAsqrMethod>(*this); }
...
OutlierInfo detectOutliers(const Eigen::MatrixXd &ResidMat, double outlierIqrRatio, double outlierAbsBound) {
    ...
    for (Eigen::Index col = 0; col < K; ++col) {
        ...
        infoMsg("  Column %d: outlier ratio = %.4f (%d / %lld)", static_cast<int>(col + 1),
                static_cast<double>(nOutlier) / N, nOutlier, static_cast<long long>(N));
    }
    return info;
}
...
static void runSPAsqrPipeline(Eigen::MatrixXd &ResidMat,
                              GenoMeta &plinkData,
                              ...
                              std::vector<std::string> tauLabels) // empty → legacy column names
{
    ...
    for (int col = 0; col < ntaus; ++col) {
        ...
        spagrm_vec.emplace_back(ResidMat.col(col), // resid
                                sum_R_nonOutlier_vec[col], R_GRM_R_nonOutlier_vec[col],
                                0.0, // R_GRM_R_TwoSubjOutlier = 0 (no families)
                                R_GRM_R_vec[col], MAF_interval, std::move(fam), spaCutoff, zeta, tol);
    }
    ...
    markerEngine(plinkData, *method, outputFile, nthreads, missingCutoff, minMafCutoff, minMacCutoff, hweCutoff);
}
...
void runSPAsqr(const std::string &residFile,
    ...
    infoMsg("Residual matrix: %lld subjects x %lld columns", static_cast<long long>(N), static_cast<long long>(K));
    ...
    runSPAsqrPipeline(ResidMat, *genoData, sd.usedIIDs(), sd.famIIDs(), genoData->nSubjUsed(), spgrmGrabFile,
                      spgrmGctaFile, outputFile, spaCutoff, outlierIqrRatio, outlierAbsBound, nthreads, missingCutoff,
                      minMafCutoff, minMacCutoff, hweCutoff, /*tauLabels=*/{});
}
...
void runSPAsqrPheno(const std::string &phenoFile,
    ...
    // ── 2. Compute bandwidth h ─────────────────────────────────────
    {
        ...
        runSPAsqrPipeline(ResidMat, *genoData, sd.usedIIDs(), sd.famIIDs(), genoData->nSubjUsed(), spgrmGrabFile,
                          spgrmGctaFile, outputFile, spaCutoff, outlierIqrRatio, outlierAbsBound, nthreads,
                          missingCutoff, minMafCutoff, minMacCutoff, hweCutoff, std::move(tauLabels));
    }
}
