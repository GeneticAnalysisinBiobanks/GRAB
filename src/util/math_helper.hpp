...
inline double pnorm(double x, double mean = 0.0, double sd = 1.0,
...
inline double qchisq(double p, double df,
                     bool lower_tail = true, bool log_p = false) {
  if (log_p) p = std::exp(p);
  if (!lower_tail) p = 1.0 - p;
  p = std::clamp(p, 1e-300, 1.0 - 1e-15);
...
inline double pt(double t, double df, bool lower_tail = true) {
  ...
  return lower_tail ? boost::math::cdf(dist, t)
                    : boost::math::cdf(boost::math::complement(dist, t));
}
...
template <typename F>
double findRootBrent(F&& f, double a, double b, double tol = 1e-8) {
  double fa = f(a);
  double fb = f(b);
  if (fa * fb > 0.0)
    throw std::runtime_error("findRootBrent: root not bracketed");
  ...
  for (int iter = 0; iter < max_iter; ++iter) {
    ...
    if (gap < tol || std::abs(fb) < tol * 10.0)
      return b;
...
template <typename F>
double brentMin(F&& f, double lo, double hi, double tol = 1e-6,
                int maxIter = 200) {
  ...
  for (int iter = 0; iter < maxIter; ++iter) {
    ...
    if (std::abs(x - midpt) <= tol2 - 0.5 * (b - a))
      return x;
