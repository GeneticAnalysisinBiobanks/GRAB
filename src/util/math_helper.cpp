...
double bvnCdf(double dh, double dk, double r) {
  // Independent case
  if (std::abs(r) < 1e-15)
    return 0.5 * std::erfc(-dh / std::sqrt(2.0))
         * 0.5 * std::erfc(-dk / std::sqrt(2.0));

  static constexpr double w6[] = {
      0.1713244923791704, 0.3607615730481386, 0.4679139345726910};
  static constexpr double x6[] = {
  ...
  if (std::abs(r) < 0.925) {
    ...
    bvn *= asr / (4.0 * M_PI);
    bvn += 0.5 * std::erfc(-h / std::sqrt(2.0))
         * 0.5 * std::erfc(-k / std::sqrt(2.0));
  } else {
    double kk = k;
    double hkk = hk;
    if (r < 0.0) { kk = -kk; hkk = -hkk; }
    ...
    if (std::abs(r) < 1.0) {
      ...
      if (-hkk < 100.0) {
        ...
        bvn -= std::exp(-hkk / 2.0) * std::sqrt(2.0 * M_PI)
             * 0.5 * std::erfc(-b / (std::sqrt(2.0) * a))
...
double pmvnorm2d(double lo1, double hi1, double lo2, double hi2,
...
Eigen::VectorXd logisticRegressionBeta(
  ...
  for (int iter = 0; iter < maxIter; ++iter) {
    ...
    Eigen::VectorXd betaNew =
        XtWX.selfadjointView<Eigen::Lower>().llt().solve(XtWz);
...
OptimResult nelderMead(
  ...
  for (int i = 0; i < nv; ++i) fvals[i] = f(simplex[i]);
  ...
  for (; iter < maxIter; ++iter) {
    ...
    if (fr < fvals[ilo]) {
      ...
      double fe = f(xe);
      if (fe < fr) { simplex[ihi] = xe; fvals[ihi] = fe; }
      else         { simplex[ihi] = xr; fvals[ihi] = fr; }
