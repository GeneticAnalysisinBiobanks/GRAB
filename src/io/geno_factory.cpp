...
static std::vector<std::string> parsePsamIIDs(const std::string& psamFile) {
    ...
    while (std::getline(ifs, line)) {
        ...
        if (!headerSeen && (toks[0] == "#FID" || toks[0] == "FID" ||
                            toks[0] == "#IID" || toks[0] == "IID")) {
            headerSeen = true;
            if (toks[0] == "#IID" || toks[0] == "IID") iidCol = 0;
            else iidCol = 1;
            continue;
        }
        headerSeen = true;
        if (static_cast<int>(toks.size()) <= iidCol)
            throw std::runtime_error("Too few columns in " + psamFile);
...
static std::vector<std::string> parseVcfSampleIDs(const std::string& vcfFile) {
    ...
    if (!hdr) { hts_close(fp); throw std::runtime_error("Cannot read header: " + vcfFile); }
...
std::unique_ptr<GenoMeta> makeGenoData(
    ...
    case GenoFormat::Pgen:
        ...
        return std::make_unique<PgenData>(
            spec.path + ".pgen", spec.path + ".pvar",
            usedMask, nSamplesInFile, nUsed,
            nMarkersEachChunk);
    case GenoFormat::Vcf:
        infoMsg("Loading VCF/BCF data: %s", spec.path.c_str());
        return std::make_unique<VcfData>(
            spec.path, usedMask, nSamplesInFile, nUsed,
            nMarkersEachChunk);
    case GenoFormat::Bgen:
        infoMsg("Loading BGEN data: %s", spec.path.c_str());
        return std::make_unique<BgenData>(
            spec.path, usedMask, nSamplesInFile, nUsed,
            nMarkersEachChunk);
    }
    throw std::runtime_error("Unknown genotype format");
