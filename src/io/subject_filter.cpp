...
static void detectFormat(const std::string& firstLine,
                          int& iidCol, bool& hasHeader) {
    if (!firstLine.empty() && firstLine[0] == '#') {
        ...
        if (!tokens.empty() && tokens[0] == "FID") {
            ...
            throw std::runtime_error(
                "subject_filter: unrecognised header '" + firstLine + "'.\n"
...
std::unordered_set<std::string> parseSubjectIDFile(const std::string& path) {
    ...
    if (!ifs)
        throw std::runtime_error("subject_filter: cannot open '" + path + "'");
    ...
    if (!hasHeader) {
        ...
        if (iidCol < (int)tokens.size())
            result.insert(tokens[(size_t)iidCol]);
    }
    ...
    const std::string& removeFile)
{
    ...
    if (!keepFile.empty()) {
        ...
        infoMsg("--keep: %u/%zu subjects in keep file matched genotype (%u not found)",
                nMatched, keepSet.size(), nUnmatched);
    }
