...
inline const FlagDef kBfile = {"--bfile", "PREFIX", "PLINK binary genotype prefix (.bed/.bim/.fam)", nullptr};

inline const FlagDef kPfile = {"--pfile", "PREFIX", "PLINK2 genotype prefix (.pgen/.pvar/.psam)", nullptr};

inline const FlagDef kVcf = {"--vcf", "FILE", "VCF or BCF genotype file", nullptr};

inline const FlagDef kBgen = {"--bgen", "FILE", "BGEN genotype file", nullptr};

// Combined display entry for genotype input (exactly one required)
inline const FlagDef kGeno_input = {"--bfile PREFIX | --pfile PREFIX | --vcf FILE | --bgen FILE", nullptr,
                                    "Genotype input (exactly one)", nullptr};

inline const FlagDef kNullResid = {"--null-resid", "FILE", "Null model residual file",
...
inline const FlagDef kOut = {"--out", "PREFIX", "Output prefix (suffix added per mode/method)", nullptr};

inline const FlagDef kCovar = {"--covar", "FILE", "Covariate file (plink2 format, columns selected by --covar-name)",
...
inline const FlagDef kPheno = {"--pheno", "FILE", "Phenotype file (plink2 format, IID + named columns)",
                               R"(Plink2-compatible IID + named columns.
...
inline const FlagDef kCovarName = {"--covar-name", "COL_IDS", "Comma-separated covariate column names",
                                   R"(Selects columns from --covar (preferred) or --pheno as covariates.
An intercept is added automatically.)"};

inline const FlagDef kCovarColNums = {"--covar-col-nums", "NUMS", "1-based column number(s)/range(s) for covariates",
...
inline const FlagDef kNotCovar = {"--not-covar", "COL_IDS", "Exclude covariate(s) by name (alias: --covarExcludeList)",
                                  nullptr};

inline const FlagDef kBinaryPheno = {"--pheno-binary", "COL", "Column name for binary phenotype (case/control)",
                                     "Selects a 0/1 column from --pheno for logistic regression."};

inline const FlagDef kSurvPheno = {"--pheno-surv", "TIME:EVENT", "Colon-separated survival time and event column names",
                                   R"(Selects survival time and event columns from --pheno for Cox regression.
Example: --pheno-surv TIME:EVENT)"};

inline const FlagDef kPcCols = {"--pc-cols", "COL_IDS", "Comma-separated PC column names (default: PC1,PC2,PC3,PC4)",
...
inline const FlagDef kRefAf = {"--ref-af", "FILE", "Reference allele frequency (plink2 .afreq or numeric)",
...
inline const FlagDef kSpGrmGrab = {"--sp-grm-grab", "FILE", "Sparse GRM, GRAB format (ID1 ID2 VALUE)",
...
inline const FlagDef kSpGrmPlink2 = {"--sp-grm-plink2", "FILE", "Sparse GRM, plink2 .grm.sp format",
...
inline const FlagDef kSpGrm = {"--sp-grm-grab FILE | --sp-grm-plink2 FILE", nullptr, "Sparse GRM (exactly one)",
                               nullptr};

inline const FlagDef kIndAfCoef = {"--ind-af-coef", "FILE", "Pre-computed individual AF model",
                                   R"(Produced by --cal-ind-af-coef.
Text/gz: #STATUS  BETA0  BETA1 ...
Rows in filtered-marker order (positional matching).)"};

inline const FlagDef kPairwiseIbd = {"--pairwise-ibd", "FILE", "Pairwise IBD file",
                                     "Tab-separated: #ID1  ID2  pa  pb  pc\n"
                                     "Produced by grab --cal-pairwise-ibd."};

// ── Scalar options ─────────────────────────────────────────────────

inline const FlagDef kPrevalence = {"--prevalence", "FLOAT", "Disease prevalence in reference population", nullptr};
inline const FlagDef kBatchPThresh = {"--batch-effect-p-threshold", "FLOAT",
                                      "Batch-effect p-value cutoff (default: 0.05)", nullptr};
inline const FlagDef kCovarPThresh = {"--covar-p-threshold", "FLOAT", "Covariate p-value cutoff (default: 5e-5)",
                                      nullptr};
inline const FlagDef kSpaZThresh = {"--spa-z-threshold", "FLOAT", "SPA z-score cutoff (default: 2.0)", nullptr};
inline const FlagDef kOutlierIqr = {"--outlier-iqr-threshold", "FLOAT", "IQR outlier multiplier (default: 1.5)",
                                    nullptr};
inline const FlagDef kOutlierAbs = {"--outlier-abs-bound", "FLOAT", "Absolute outlier cutoff (default: 0.55)", nullptr};
inline const FlagDef kThreads = {"--threads", "INT", "Worker threads (default: 1)", nullptr};
inline const FlagDef kChunkSize = {"--chunk-size", "INT", "Markers per chunk (default: 8192, min: 256)", nullptr};
inline const FlagDef kCompression = {"--compression", "gz|zst", "Compress output files (default: plain text)", nullptr};
inline const FlagDef kCompressionLevel = {
    "--compression-level", "INT", "Compression level (gz: 1-9, zst: 1-22; default: 0 = library default)", nullptr};
inline const FlagDef kGeno = {"--geno", "FLOAT", "Per-marker missing rate cutoff (default: 0.1)", nullptr};
inline const FlagDef kMaf = {"--maf", "FLOAT", "Min minor allele frequency (default: 1e-5)", nullptr};
inline const FlagDef kMac = {"--mac", "FLOAT", "Min minor allele count (default: 10)", nullptr};
inline const FlagDef kHwe = {"--hwe", "FLOAT", "Exclude markers with HWE p < threshold (default: 0, disabled)",
                             nullptr};
inline const FlagDef kMinMafIbd = {"--min-maf-ibd", "FLOAT", "Min MAF for IBD calculation (default: 0.01)", nullptr};
inline const FlagDef kNClusters = {"--leaf-nclusters", "INT",
                                   "Number of K-means clusters for LEAF (default: from --ref-af count)", nullptr};

inline const FlagDef kKeep = {"--keep", "FILE", "Restrict analysis to subjects listed in file (FID IID per line)",
                              nullptr};
inline const FlagDef kRemove = {"--remove", "FILE", "Exclude subjects listed in file (FID IID per line)", nullptr};

inline const FlagDef kExtract = {"--extract", "FILE", "Restrict analysis to SNPs listed in file (one ID per line)",
                                 R"(Single-column file of SNP IDs (matching .bim column 2).
Applied to both --bfile and --admix-bfile inputs.)"};
inline const FlagDef kExclude = {"--exclude", "FILE", "Exclude SNPs listed in file from analysis (one ID per line)",
                                 R"(Single-column file of SNP IDs (matching .bim column 2).
Applied to both --bfile and --admix-bfile inputs.)"};
...
inline const FlagDef kAdmixPhi = {"--admix-phi", "FILE", "Pre-computed phi file from --cal-phi (wide format)",
...
inline const FlagDef kMsp = {"--rfmix-msp", "FILE", "MSP local-ancestry file from rfmix2 (for --make-abed)",
...
;
inline const FlagDef kSeed = {"--seed", "INT", "Random seed for reproducibility (default: 0 = use random device)",
                              nullptr};
...
// ── SPACox ─────────────────────────────────────────────────────────
inline const FlagDef *const kSPACoxReq[] = {&kGeno_input, &kNullResid, &kOut, nullptr};
inline const FlagDef *const kSPACoxOpt[] = {
    &kCovar,       &kCovarName,        &kCovarColNums, &kNotCovar, &kCovarPThresh, &kSpaZThresh, &kThreads, &kChunkSize,
    &kCompression, &kCompressionLevel, &kGeno,         &kMaf,      &kMac,          &kHwe,        nullptr};
inline const MethodDef kSPACox = {"SPACox",
                                  "Saddlepoint Approximation for Cox proportional hazards",
                                  kSPACoxReq,
                                  kSPACoxOpt,
                                  "#IID  RESID  [RESID2  ...]",
                                  "Per residual column: PREFIX.PHENO.SPACox[.gz|.zst]\n"
                                  "  CHROM  POS  ID  REF  ALT  MISS_RATE  ALT_FREQ  MAC  HWE_P  P  Z",
                                  nullptr};

// ── SPAGRM ─────────────────────────────────────────────────────────
inline const FlagDef *const kSPAGRMReq[] = {&kGeno_input, &kNullResid, &kOut, &kSpGrm, &kPairwiseIbd, nullptr};
inline const FlagDef *const kSPAGRMOpt[] = {&kSpaZThresh, &kThreads, &kChunkSize, &kCompression, &kCompressionLevel,
                                            &kGeno,       &kMaf,     &kMac,       &kHwe,         nullptr};
inline const MethodDef kSPAGRM = {"SPAGRM",
                                  "SPA with sparse GRM relatedness correction",
                                  kSPAGRMReq,
                                  kSPAGRMOpt,
                                  "#IID  RESID  [RESID2  ...]",
                                  "Per residual column: PREFIX.PHENO.SPAGRM[.gz|.zst]\n"
                                  "  CHROM  POS  ID  REF  ALT  MISS_RATE  ALT_FREQ  MAC  HWE_P  P  Z",
                                  "Generate pairwise IBD file with: grab --cal-pairwise-ibd"};

// ── SAGELD ─────────────────────────────────────────────────────────
inline const FlagDef *const kSAGELDReq[] = {&kGeno_input, &kNullResid, &kOut, &kSpGrm, &kPairwiseIbd, nullptr};
inline const FlagDef *const kSAGELDOpt[] = {&kSpaZThresh, &kThreads, &kChunkSize, &kGeno, &kMaf, &kMac, &kHwe, nullptr};
inline const MethodDef kSAGELD = {"SAGELD",
...
// ── SPAmix ─────────────────────────────────────────────────────────
inline const FlagDef *const kSPAmixReq[] = {&kGeno_input, &kNullResid, &kOut, &kPcCols, nullptr};
inline const FlagDef *const kSPAmixOpt[] = {&kPheno,      &kCovar,   &kIndAfCoef, &kSpGrm,       &kOutlierIqr,
                                            &kSpaZThresh, &kThreads, &kChunkSize, &kCompression, &kCompressionLevel,
                                            &kGeno,       &kMaf,     &kMac,       &kHwe,         nullptr};
inline const MethodDef kSPAmix = {"SPAmix",
                                  "SPA with individual ancestry-based allele frequencies",
                                  kSPAmixReq,
                                  kSPAmixOpt,
                                  "#IID  RESID  [RESID2  ...]",
                                  "Per residual column: PREFIX.PHENO.SPAmix[.gz|.zst]\n"
...
// ── SPAmixPlus ─────────────────────────────────────────────────────
inline const FlagDef *const kSPAmixPlusReq[] = {&kGeno_input, &kNullResid, &kOut, &kPcCols, &kSpGrm, nullptr};
inline const FlagDef *const kSPAmixPlusOpt[] = {
    &kPheno,     &kCovar,       &kIndAfCoef,        &kOutlierIqr, &kSpaZThresh, &kThreads,
    &kChunkSize, &kCompression, &kCompressionLevel, &kGeno,       &kMaf,        &kMac,
    &kHwe,       nullptr};
inline const MethodDef kSPAmixPlus = {"SPAmixPlus",
...
// ── POLMM ──────────────────────────────────────────────────────────
inline const FlagDef *const kPOLMMReq[] = {&kGeno_input, &kPheno, &kOrdinalPheno, &kOut, &kSpGrm, nullptr};
inline const FlagDef *const kPOLMMOpt[] = {&kCovarName, &kCovarColNums, &kNotCovar, &kSpaZThresh,
...
// ── WtCoxG ─────────────────────────────────────────────────────────
inline const FlagDef *const kWtCoxGReq[] = {&kGeno_input, &kOut, &kRefAf, &kPrevalence, nullptr};
inline const FlagDef *const kWtCoxGOpt[] = {&kNullResid,  &kPheno,       &kCovar,     &kCovarName, &kCovarColNums,
                                            &kNotCovar,   &kBinaryPheno, &kSurvPheno, &kSpGrm,     &kBatchPThresh,
                                            &kSpaZThresh, &kThreads,     &kChunkSize, &kGeno,      &kMaf,
                                            &kMac,        &kHwe,         nullptr};
inline const MethodDef kWtCoxG = {"WtCoxG",
...
// ── LEAF ───────────────────────────────────────────────────────────
inline const FlagDef *const kLEAFReq[] = {&kGeno_input, &kOut, &kRefAf, &kPrevalence, nullptr};
inline const FlagDef *const kLEAFOpt[] = {
    &kNullResid, &kPheno,     &kCovar,     &kCovarName, &kCovarColNums, &kNotCovar,     &kBinaryPheno,
    &kSurvPheno, &kPcCols,    &kNClusters, &kSeed,      &kSpGrm,        &kBatchPThresh, &kSpaZThresh,
    &kThreads,   &kChunkSize, &kGeno,      &kMaf,       &kMac,          &kHwe,          nullptr};
...
inline const FlagDef *const kCalPhiReq[] = {&kAdmixBfile, &kSpGrm, &kOut, nullptr};
inline const FlagDef *const kCalPhiOpt[] = {&kKeep,        &kRemove,           &kExtract, &kExclude,
                                            &kCompression, &kCompressionLevel, nullptr};
inline const MethodDef kCalPhi = {"cal-phi",
...
inline const FlagDef *const kSPAmixLocalPlusReq[] = {&kAdmixBfile, &kNullResid, &kAdmixPhi, &kOut, nullptr};
inline const FlagDef *const kSPAmixLocalPlusOpt[] = {
    &kExtract,          &kExclude, &kOutlierIqr, &kSpaZThresh, &kThreads, &kChunkSize, &kCompression,
    &kCompressionLevel, &kGeno,    &kMaf,        &kMac,        &kHwe,     nullptr};
inline const MethodDef kSPAmixLocalPlus = {"SPAmixLocalPlus",
...
inline const MethodDef kMakeAbed = {"make-abed",
...
inline const FlagDef *const kCalAfReq[] = {&kGeno_input, &kPcCols, &kOut, nullptr};
inline const FlagDef *const kCalAfOpt[] = {&kPheno,   &kCovar,     &kKeep, &kRemove, &kCompression, &kCompressionLevel,
                                           &kThreads, &kChunkSize, &kGeno, &kMaf,    &kMac,         &kHwe,
                                           nullptr};
inline const MethodDef kCalAfCoef = {
    "cal-af-coef", "Compute per-marker individual-ancestry AF model",
    kCalAfReq,     kCalAfOpt,
    nullptr,       "Output: PREFIX.afc[.gz|.zst] (pass to --ind-af-coef for SPAmix/SPAmixPlus)",
    nullptr};

// ── Utility mode: cal-pairwise-ibd ────────────────────────────────
inline const FlagDef *const kCalIbdReq[] = {&kGeno_input, &kOut, &kSpGrm, nullptr};
inline const FlagDef *const kCalIbdOpt[] = {&kKeep, &kRemove, &kMinMafIbd, &kCompression, &kCompressionLevel, nullptr};
inline const MethodDef kCalPairwiseIbd = {"cal-pairwise-ibd",
...
inline const MethodDef *const kAllMethods[] = {&kSPACox, &kSPAGRM, &kSAGELD, &kSPAmix, &kSPAmixPlus, &kSPAmixLocalPlus,
                                               &kPOLMM,  &kSPAsqr, &kWtCoxG, &kLEAF,   nullptr};

inline const MethodDef *const kAllUtilModes[] = {&kCalAfCoef, &kCalPairwiseIbd, &kCalPhi, &kMakeAbed, nullptr};

// File-accepting flags (for --help <flag-topic>)
inline const FlagDef *const kFileFlags[] = {&kNullResid,   &kPheno,     &kCovar,       &kRefAf,    &kSpGrmGrab,
                                            &kSpGrmPlink2, &kIndAfCoef, &kPairwiseIbd, &kAdmixPhi, nullptr};

// All flags grouped for --help options
inline const FlagDef *const kInputFlags[] = {&kBfile,       &kPfile,       &kVcf,
                                             &kBgen,        &kAdmixBfile,  &kNullResid,
                                             &kOut,         &kCompression, &kCompressionLevel,
                                             &kPheno,       &kCovar,       &kCovarName,
                                             &kBinaryPheno, &kSurvPheno,   &kOrdinalPheno,
...
inline const FlagDef *const kNumericFlags[] = {
    ...
    &kMaf,        &kMac,          &kMinMafIbd,        nullptr};

inline const FlagDef *const kFilterFlags[] = {&kExtract, &kExclude, nullptr};
