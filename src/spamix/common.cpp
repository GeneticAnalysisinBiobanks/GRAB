...
RootResult fastGetRootK1(
  ...
  for (int iter = 0; iter < maxiter; ++iter) {
    ...
    auto [k1_adj, k2_val] = evalOutlierK1adjK2(
        x, s, residOutlier, mafOutlier, nOutlier);
...
OutlierData detectOutliers(const Eigen::VectorXd& resid, double outlierRatio) {
  ...
  for (uint32_t idx : posOutlier) isOutlier[idx] = true;
  std::vector<uint32_t> posNonOutlier;
  posNonOutlier.reserve(static_cast<size_t>(N) - posOutlier.size());
  for (int i = 0; i < N; ++i)
    if (!isOutlier[i])
      posNonOutlier.push_back(static_cast<uint32_t>(i));
...
double spa::getProbSpaG(
  ...
  auto rootRes = fastGetRootK1(
      s, mafOutlier, residOutlier, nOutlier,
      mean_nonOutlier, var_nonOutlier);
  double zeta = rootRes.root;

  auto [k0_outlier, k2_outlier] = evalOutlierK0K2(
      zeta, residOutlier, mafOutlier, nOutlier);
  double k0_total = k0_outlier
                  + mean_nonOutlier * zeta
                  + 0.5 * var_nonOutlier * zeta * zeta;
  double k2_total = k2_outlier + var_nonOutlier;

  double temp1 = zeta * s - k0_total;

  if (!std::isfinite(zeta) || temp1 < 0.0 || k2_total <= 0.0)
