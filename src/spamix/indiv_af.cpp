...
static std::vector<AFModel>
loadAFModelsBinary(const std::string &path, int nPC, uint32_t nMarkers, const std::vector<uint64_t> &genoIndices) {
...
static std::vector<AFModel> loadAFModelsText(const std::string &path, int nPC, uint32_t nExpected) {
    ...
    auto parseLine = [&](std::string line) {
        ...
        int status;
        if (!(iss >> status))
            throw std::runtime_error(path + " line " + std::to_string(lineNo) + ": expected STATUS BETA0 ...");
        AFModel m;
        m.status = static_cast<int8_t>(status);
        m.betas.resize(1 + nPC);
        for (int j = 0; j <= nPC; ++j) {
            if (!(iss >> m.betas[j]))
                throw std::runtime_error(path + " line " + std::to_string(lineNo) + ": expected " +
...
loadAFModels(const std::string &path, int nPC, uint32_t nMarkers, const std::vector<uint64_t> &genoIndices) {
    ...
    if (models.size() != nMarkers)
        throw std::runtime_error("AF model count (" + std::to_string(models.size()) +
