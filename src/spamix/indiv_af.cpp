...
AFModel computeAFModel(
  ...
  for (int j = 0; j < nPC; ++j) {
    ...
    if (2.0 * math::pt(-std::abs(t), df) < PCs_pval_cut)
      sigIdx.push_back(j);
  }
                 ...
                 static_cast<long long>(1 + nPC) * static_cast<long long>(sizeof(double)))
{
  if (m_mode == Mode::Binary) {
...

    {
      ...
      if (!pre.is_open())
        throw std::runtime_error("Cannot create binary AF file: " + outputFile);
      constexpr std::streamsize CHUNK = 1 << 20;
      std::vector<char> zeros(static_cast<size_t>(
          std::min<long long>(CHUNK, totalSize > 0 ? totalSize : CHUNK)), 0);
      for (long long rem = totalSize; rem > 0; ) {
        const std::streamsize n = static_cast<std::streamsize>(
            std::min<long long>(rem, static_cast<long long>(zeros.size())));
...
void IndivAFWriter::write(
  ...
  if (m_mode == Mode::Binary) {
    ...
    m_binOut.write(reinterpret_cast<const char*>(betas.data()),
...
IndivAFReader::~IndivAFReader() { m_in.close(); }

bool IndivAFReader::read(uint64_t genoIndex, AFModel& model) {
  const long long filePos = static_cast<long long>(genoIndex) * m_recordSize;
  m_in.seekg(filePos);
  if (!m_in.good())
    throw std::runtime_error("Binary AF seek failed at genoIndex=" +
  ...
  m_in.read(reinterpret_cast<char*>(model.betas.data()),
            static_cast<std::streamsize>((1 + m_nPC) * sizeof(double)));
  if (!m_in.good())
...
std::vector<AFModel> computeAFModelsInMemory(
  ...
  workers.reserve(nt);
  for (int i = 0; i < nt; ++i) workers.emplace_back(workerFn);
