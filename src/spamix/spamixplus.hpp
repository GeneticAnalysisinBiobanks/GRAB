...
class SPAmixPlusMethod : public MethodBase {
  public:
    ...
    // Pre-computed AF + GRM
    SPAmixPlusMethod(const Eigen::VectorXd &residuals,
    ...
    std::string getHeaderColumns() const override { return "\tP\tZ\tBETA"; }
  ...
  private:
    double
    getMarkerPval(const Eigen::Ref<const Eigen::VectorXd> &GVec, double altFreq, double &zScore, double &outVarS);
