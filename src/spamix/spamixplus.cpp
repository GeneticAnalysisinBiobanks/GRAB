...
// Pre-computed AF + GRM
SPAmixPlusMethod::SPAmixPlusMethod(const Eigen::VectorXd &residuals,
...
std::unique_ptr<MethodBase> SPAmixPlusMethod::clone() const {
    if (m_hasGRM) {
        if (m_XtX_inv_Xt) {
            return std::make_unique<SPAmixPlusMethod>(m_resid, m_resid2, m_onePlusPCs, m_outlier, m_spaCutoff, *m_grm,
    ...
    // No GRM
    if (m_XtX_inv_Xt) {
        return std::make_unique<SPAmixPlusMethod>(m_resid, m_resid2, m_onePlusPCs, m_outlier, m_spaCutoff,
                                                  *m_XtX_inv_Xt, *m_sqrt_XtX_inv_diag, m_nPC);
    }
    return std::make_unique<SPAmixPlusMethod>(m_resid, m_resid2, m_onePlusPCs, m_outlier, m_spaCutoff, *m_afModels,
                                              *m_genoToFlat);
}

void SPAmixPlusMethod::prepareChunk(const std::vector<uint64_t> &gIndices) { m_chunkGenoIndices = gIndices; }

// ======================================================================
// getMarkerPval — score test with optional GRM variance + SPA
// ======================================================================

double SPAmixPlusMethod::getMarkerPval(const Eigen::Ref<const Eigen::VectorXd> &GVec,
                                       double altFreq,
                                       double &zScore,
                                       double &outVarS) {
    ...
    double pval1 = spa::getProbSpaG(m_mafOutlier.data(), m_outlier.residOutlier.data(), nOut, absS + S_mean_spa, false,
                                    mean_nonOutlier, var_nonOutlier);

    double pval2 = spa::getProbSpaG(m_mafOutlier.data(), m_outlier.residOutlier.data(), nOut, -absS + S_mean_spa, true,
                                    mean_nonOutlier, var_nonOutlier);
...
void SPAmixPlusMethod::getResultVec(Eigen::Ref<Eigen::VectorXd> GVec,
                                    double altFreq,
...
void runSPAmixPlus(const std::string &residFile,
    ...
    for (int rc = 0; rc < K; ++rc) {
        ...
        if (hasGRM) {
            if (!afFile.empty()) {
                m = std::make_unique<SPAmixPlusMethod>(pResid[rc], pResid2[rc], pOnePlusPCs[rc], pOutlier[rc],
                                                       spaCutoff, *grmPtr, afModels, genoToFlat);
            } else {
                m = std::make_unique<SPAmixPlusMethod>(pResid[rc], pResid2[rc], pOnePlusPCs[rc], pOutlier[rc],
                                                       spaCutoff, *grmPtr, pXtX_inv_Xt[rc], pSqrt_XtX_inv_diag[rc],
                                                       nPC);
            }
        } else {
            if (!afFile.empty()) {
                ...
                m = std::make_unique<SPAmixPlusMethod>(pResid[rc], pResid2[rc], pOnePlusPCs[rc], pOutlier[rc],
                                                       spaCutoff, pXtX_inv_Xt[rc], pSqrt_XtX_inv_diag[rc], nPC);
            }
        }
    ...
    multiPhenoEngine(*genoData, tasks, outPrefix, methodLabel, compression, compressionLevel, nthread, missingCutoff,
