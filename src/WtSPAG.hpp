...
private:
  ...
  arma::vec m_N_ref;             // reference MAC   
...
public:
  ...
  void set_N_ref(arma::vec t_N_ref){m_N_ref = t_N_ref;}
                       ...
                       int t_i)
  {
    ...
    double AF = (m_N * t_altFreq + N_ref * AF_ref) / (m_N + N_ref);
    double S = sum(m_mresid % (t_GVec - 2 * AF));
    
    double G_var = 2 * AF * (1 - AF);
    
    double meanR = mean(m_mresid);
    double tildeR = meanR * m_N / (m_N + N_ref);
    
    double S_var = (sum(pow(m_mresid - tildeR, 2)) + N_ref * pow(tildeR, 2)) * G_var;
    ...
    double mean_nonOutlier = (sum(residNonOutlier-tildeR) - N_ref * tildeR ) *2*AF;
    double var_nonOutlier = (sum(pow((residNonOutlier-tildeR), 2) + N_ref * pow(tildeR, 2))) * G_var;
