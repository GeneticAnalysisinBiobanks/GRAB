...
double WtCoxGClass::getProbSpaG(
  ...
  auto h1_func = [&](double t) {
    return h1Adj(t, R, s, MAF, n_ext, N_all, sumR, var_mu_ext,
              g_var_est, meanR, b);
  };
  ...
  double k1 = hOrg(zeta, R, MAF, n_ext, N_all, sumR, var_mu_ext,
              g_var_est, meanR, b);
  double k2 = h2(zeta, R, MAF, n_ext, N_all, sumR, var_mu_ext,
            g_var_est, meanR, b);
...
arma::vec WtCoxGClass::spaGOneSnpHomo(
  ...
  if (std::abs(z) < SPA_Cutoff) {
    ...
    double pval1 = getProbSpaG(MAF, R, std::abs(S), n_ext, N_all, sumR,
                      var_mu_ext, g_var_est, meanR, b, false);
    double pval2 = getProbSpaG(MAF, R, -std::abs(S), n_ext, N_all, sumR,
...
arma::vec WtCoxGClass::wtCoxGTest(
  ...
  arma::vec spa_result_s1 = spaGOneSnpHomo(
