...
void adjustGenotype(const double *G, const uint32_t *nzIdx, int nNz, Eigen::Ref<Eigen::VectorXd> adjG) const;
...
SPACoxMethod(const Eigen::VectorXd &residuals,
             double varResid,
...
double evalK0(double t, int N0, double adjG0, const double *adjG, const uint32_t *idx, int n) const;
// Fused K1+K2 evaluation — single loop, one interp lookup per element
std::pair<double, double>
evalK1K2(double t, int N0, double adjG0, const double *adjG, const uint32_t *idx, int n, double q2) const;
...
RootResult
fastGetRootK1(double initX, int N0, double adjG0, const double *adjG, const uint32_t *idx, int n, double q2) const;
double
getProbSpa(double adjG0, const double *adjG, const uint32_t *idx, int n, int N0, double q2, bool lowerTail) const;

// ---- Per-marker score test ----
double getMarkerPval(const Eigen::Ref<Eigen::VectorXd> &GVec, double MAF, double &zScore);
...
void runSPACox(const std::vector<std::string> &residNames,
