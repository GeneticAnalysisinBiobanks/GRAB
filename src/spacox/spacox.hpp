...
private:
  ...
  RootResult fastGetRootK1(double initX, int N0, double adjG0,
                           const double* adjG, const uint32_t* idx, int n,
                           double q2) const;
  double getProbSpa(double adjG0, const double* adjG, const uint32_t* idx, int n,
                    int N0, double q2, bool lowerTail) const;

  // ---- Per-marker score test ----
  double getMarkerPval(const Eigen::Ref<Eigen::VectorXd>& GVec,
