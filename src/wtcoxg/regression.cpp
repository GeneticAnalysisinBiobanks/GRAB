...
Eigen::VectorXd calRegrWeight(double prevalence,
  ...
  if (nCase < 1 || nCtrl < 1)
    throw std::runtime_error("calRegrWeight: need at least 1 case and 1 control");
  if (prevalence <= 0.0 || prevalence >= 1.0)
    throw std::runtime_error("calRegrWeight: prevalence must be in (0, 1)");
...
void parseSurvSpec(const std::string& spec,
  ...
  if (pos == std::string::npos || pos == 0 || pos == spec.size() - 1)
    throw std::runtime_error(
        "Invalid --pheno-survival format: expected TIME:EVENT, got '" + spec + "'");
...
std::vector<Eigen::Index> completeRows(
  ...
  for (Eigen::Index i = 0; i < n; ++i) {
    bool ok = true;
    for (auto* v : vecs)
      if (std::isnan((*v)[i])) { ok = false; break; }
    if (!ok) continue;
    for (Eigen::Index j = 0; j < X.cols(); ++j)
      if (std::isnan(X(i, j))) { ok = false; break; }
...
Eigen::MatrixXd subsetRows(const Eigen::Ref<const Eigen::MatrixXd>& M,
...
Eigen::VectorXd subsetVec(const Eigen::Ref<const Eigen::VectorXd>& v,
                          const std::vector<Eigen::Index>& idx) {
...
void validateSubset(const std::string& fname,
  ...
  if (n == 0)
    throw std::runtime_error(fname + ": no complete cases after NaN removal");
  if (n <= p)
    throw std::runtime_error(fname + ": n (" + std::to_string(n) +
        ") must exceed p (" + std::to_string(p) +
        ") — too few subjects for the number of covariates");
  for (Eigen::Index i = 0; i < n; ++i) {
    if (!(w[i] > 0.0))
      throw std::runtime_error(fname + ": non-positive weight (" +
          std::to_string(w[i]) + ") at row " + std::to_string(i));
  }
  for (Eigen::Index j = startCol; j < p; ++j) {
    if (X.col(j).maxCoeff() - X.col(j).minCoeff() < 1e-14)
      throw std::runtime_error(fname + ": covariate column " +
          std::to_string(j) + " has zero variance"
...
Eigen::VectorXd coxResiduals(
  ...
  if (d_.sum() < 1.0)
    throw std::runtime_error("coxResiduals: no observed events (all censored)");
  ...
  for (int iter = 0; iter < maxIter; ++iter) {
    ...
    auto advanceS2 = [&](Eigen::Index target) {
      for (Eigen::Index l = s2_built_to + 1; l <= target; ++l) {
        ...
        S2_cum.selfadjointView<Eigen::Lower>().rankUpdate(
            Xs.row(l).transpose(), wt);
      }
      s2_built_to = target;
    };
    ...
    if (iter > 0) {
      ...
      double relChange = (loglikOld != 0.0)
          ? absChange / std::fabs(loglikOld) : absChange;
      if (relChange < tol || absChange < sqrtTol)
        break;
    }
    ...
    if (p > 0) {
      ...
      if (llt.info() == Eigen::NumericalIssue)
        throw std::runtime_error(
            "coxResiduals: information matrix is singular at iteration " +
            std::to_string(iter) +
            " (collinearity or complete separation)");
      beta += llt.solve(U);
    }
  }
...
Eigen::VectorXd logisticResiduals(
  ...
  validateSubset("logisticResiduals", nk, p, ws, Xs, /*startCol=*/1);
  {
    ...
    if (nCase < 1.0)
      throw std::runtime_error(
          "logisticResiduals: no cases (y=1) after NaN removal");
    if (static_cast<double>(nk) - nCase < 1.0)
      throw std::runtime_error(
          "logisticResiduals: no controls (y=0) after NaN removal");
  }
  ...
  for (int iter = 0; iter < maxIter; ++iter) {
    ...
    if (llt.info() == Eigen::NumericalIssue)
      throw std::runtime_error(
          "logisticResiduals: weighted information matrix is singular at iteration " +
          std::to_string(iter) +
          " (collinearity or complete separation)");
