...
double getProbSpaG(double MAF,
                   ...
                   double b,
                   bool lower_tail) {
    auto h1_func = [&](double t) { return h1Adj(t, R, s, MAF, obs_ct, N_all, sumR, var_mu_ext, g_var_est, meanR, b); };
    double zeta;
    try {
        double a = -1.0, bb = 1.0;
...
SpaResult spaGOneSnpHomo(const Eigen::Ref<const Eigen::VectorXd> &g,
                         const Eigen::VectorXd &R,
                         double mu_ext,
                         double obs_ct,
                         double b,
...
std::vector<RefAfRecord> loadRefAfFile(const std::string &filename, bool *isNumericFallback) {
    ...
    if (colChrom < 0 || colId < 0 || colRef < 0 || colAlt < 0 || colAltFreqs < 0 || colObsCt < 0) {
        ...
        throw std::runtime_error(filename + ": missing required header columns "
    ...
    auto parseLine = [&](const std::string &ln) {
        ...
        if (static_cast<int>(tokens.size()) <= maxCol)
            throw std::runtime_error(filename + " line " + std::to_string(lineNo) + ": expected at least " +
        ...
        if (endPtr == tokens[colAltFreqs].c_str())
            throw std::runtime_error(filename + " line " + std::to_string(lineNo) + ": invalid ALT_FREQS value");
        r.obs_ct = std::strtod(tokens[colObsCt].c_str(), &endPtr);
        if (endPtr == tokens[colObsCt].c_str())
            throw std::runtime_error(filename + " line " + std::to_string(lineNo) + ": invalid OBS_CT value");
        recs.push_back(std::move(r));
    };
...
std::vector<MatchedMarkerInfo> matchMarkersNumeric(const GenoMeta &plinkData, const std::vector<RefAfRecord> &refAf) {
    ...
    if (refAf.size() != markers.size())
        throw std::runtime_error("ref-af numeric fallback: row count (" + std::to_string(refAf.size()) +
    ...
    for (size_t grp = 0; grp + 1 < mafBreaks.size(); ++grp) {
        ...
        auto opti_fun = [&](const std::vector<double> &par) -> double {
            ...
            for (int j = 0; j < nCut; ++j) {
                ...
                double pro_cut =
                    par[0] * (std::exp(d_val) * (std::exp(c_val - d_val) - 1.0)) + (1.0 - par[0]) * (1.0 - p_cut);
...
WtCoxGMethod::WtCoxGMethod(Eigen::VectorXd R,
                           Eigen::VectorXd w,
                           double cutoff,
                           double SPA_Cutoff,
...
std::string WtCoxGMethod::getHeaderColumns() const { return "\tp_ext\tp_noext\tz_ext\tz_noext\tp_batch"; }
...
void WtCoxGMethod::getResultVec(Eigen::Ref<Eigen::VectorXd> GVec,
                                double /*altFreq*/,
    ...
    WtResult res_ext =
        wtCoxGTest(GVec, info.pvalue_bat, info.TPR, info.sigma2, info.w_ext, info.var_ratio_int, info.var_ratio_w0,
                   info.var_ratio_w0, info.var_ratio_ext, info.var_ratio_ext, info.AF_ref, info.obs_ct, m_cutoff);

    // Without external reference
    WtResult res_noext = wtCoxGTest(GVec, info.pvalue_bat, NaN::quiet_NaN(), NaN::quiet_NaN(), 0.0, info.var_ratio_int,
                                    1.0, 1.0, 1.0, 1.0, NaN::quiet_NaN(), NaN::quiet_NaN(), m_cutoff);
...
WtCoxGMethod::WtResult WtCoxGMethod::wtCoxGTest(const Eigen::Ref<const Eigen::VectorXd> &g_input,
                                                double p_bat,
                                                double TPR,
                                                double sigma2,
                                                double b,
    ...
    double negInf = -std::numeric_limits<double>::infinity();
    double p0 = math::pmvnorm2d(negInf, -std::abs(S / std::sqrt(var_ratio0)), lb / std::sqrt(var_ratio_w0),
                                ub / std::sqrt(var_ratio_w0), var_S, cov_val, var_Sbat);
    p0 = std::clamp(p0, 0.0, 1.0);
    ...
    double p1 = math::pmvnorm2d(negInf, -std::abs(S / std::sqrt(var_ratio1)), lb / std::sqrt(var_ratio_w1),
                                ub / std::sqrt(var_ratio_w1), var_S1, cov_val1, var_Sbat1);
    p1 = std::clamp(p1, 0.0, 1.0);
...
void runWtCoxG(const std::string &residFile,
               const GenoSpec &geno,
    ...
    WtCoxGMethod method(sd.residuals(), sd.weights(), cutoff, spaCutoff, refInfoMap);
