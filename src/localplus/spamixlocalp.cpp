...
void computeVarOffSoABatch(
    const RprodSoA &rp, const uint32_t *hIntSM, uint32_t /*nUsed*/, int batchLen, double *varOff) {
    const size_t N = rp.size();
...
PhiMatrices estimatePhiOneAncestry(const AdmixData &admixData, const SparseGRM &grm, int ancIdx, int nthreads) {
    ...
    auto workerFn = [&](int workerId, uint32_t mBegin, uint32_t mEnd) -> ThreadResult {
        ...
        for (uint32_t m = mBegin; m < mEnd; ++m) {
            ...
            for (size_t p = 0; p < nPairs; ++p) {
                ...
                if (checkMissing) {
                    if (!std::isfinite(hi) || !std::isfinite(hj) || !std::isfinite(gi) || !std::isfinite(gj)) continue;
                }
            ...
            if (finished % 10000 == 0 || finished == total)
                infoMsg("  worker %d finished %u/%u markers", workerId, finished, total);
        }
        return res;
    };
    ...
    if (nthreads == 1) {
        ...
        for (int t = 0; t < nthreads - 1; ++t) {
            uint32_t mBegin = t * chunkSize;
            uint32_t mEnd = std::min(mBegin + chunkSize, nMarkers);
            workers.emplace_back([&results, &workerFn, t, mBegin, mEnd]() { results[t] = workerFn(t, mBegin, mEnd); });
        }
    ...
    infoMsg("  Phi entries: A=%zu, B=%zu, C=%zu, D=%zu", result.A.size(), result.B.size(), result.C.size(),
            result.D.size());
...
double lugannamiRicePval(double zeta,
                         double s,
                         double q,
                         const double *rOut,
                         const double *hOut,
...
static void writePhiWide(const std::string &path, const std::vector<PhiMatrices> &allPhi, int K) {
    ...
    auto pairHash = [](const PairKey &p) { return std::hash<uint64_t>{}(uint64_t(p.i) << 32 | p.j); };
    auto pairEq = [](const PairKey &a, const PairKey &b) { return a.i == b.i && a.j == b.j; };
    ...
    auto insertEntries = [&](const std::vector<PhiEntry> &entries, int colIdx) {
        for (const auto &e : entries) {
            auto [it, inserted] =
                rows.try_emplace({e.i, e.j}, std::vector<double>(nCols, std::numeric_limits<double>::quiet_NaN()));
...
static std::vector<PhiMatrices> readPhiWide(const std::string &path, int K) {
    ...
    for (int k = 0; k < K; ++k)
        infoMsg("  anc%d phi: A=%zu B=%zu C=%zu D=%zu", k, result[k].A.size(), result[k].B.size(), result[k].C.size(),
                result[k].D.size());
...
static void runUnifiedGWAS(const AdmixData &admixData,
    ...
    auto workerFn = [&]() {
        ...
        for (size_t ci = nextChunk.fetch_add(1); ci < nChunks; ci = nextChunk.fetch_add(1)) {
            ...
            for (size_t mi = 0; mi < gIndices.size(); mi += PHI_BATCH) {
                int batchLen = static_cast<int>(std::min(static_cast<size_t>(PHI_BATCH), gIndices.size() - mi));
                ...
                for (int b = 0; b < batchLen; ++b) {
                    ...
                    for (int k = 0; k < K; ++k) {
                        ...
                        if (as.missRate > missingCutoff || as.maf < mafCutoff || as.maf > (1.0 - mafCutoff) ||
                            as.mac < macCutoff) {
                            as.pass = false;
                ...
                for (int b = 0; b < batchLen; ++b) {
                    ...
                    for (int k = 0; k < K; ++k) {
                        ...
                        auto hapCol = bHap[b].col(k);
                        auto [pSpa, pNorm] =
                            spaLocalPval(as.S, as.sMean, as.diagVar, resid, hapCol, as.q, varS, outlier, spaCutoff);
                        double betaG =
                            (varS > 0.0) ? (as.S - as.sMean) / varS : std::numeric_limits<double>::quiet_NaN();
            ...
            if (nChunks >= 20) {
                ...
                if (done == q1 || done == q2 || done == q3) {
                    uint32_t markersDone = static_cast<uint32_t>(static_cast<uint64_t>(done) * nMarkers / nChunks);
                    infoMsg("    %u / %u markers (~%u%%)", markersDone, nMarkers,
...
void runSPAmixLocalPlus(const std::string &residFile,
    ...
    for (int rc = 0; rc < nRC; ++rc) {
        ...
        infoMsg("  Phenotype '%s': %u subjects, %u markers, %d ancestries -> %s", pi.name.c_str(), pi.nUsed,
                admixData.nMarkers(), K, outFile.c_str());

        // Detect outliers per residual column
        OutlierData outlier = detectOutliers(resid, outlierRatio);

        runUnifiedGWAS(admixData, resid, allPhi, outlier, spaCutoff, missingCutoff, minMafCutoff, minMacCutoff, outFile,
