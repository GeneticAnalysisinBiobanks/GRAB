                    ...
                    MarkerIDs = NULL)
{
  ...
  if(any(!SampleIDs %in% samples))
    stop("At least one sample from 'SampleIDs' are not in 'GenoFile'.")
  
  if(any(!MarkerIDs %in% markers))
    stop("At least one marker from 'MarkerIDs' are not in 'GenoFile'.")
  
  GenoMat = getGenoInR(MarkerIDs)
                        ...
                        SampleIDs = NULL)
{
  ...
  # if the genotype input is plink format
  if(GenoFileExt == "bed"){
    genoType = "PLINK"
  ...
  # return genotype
  print(paste("Based on the 'GenoFile', we are using a genotype file with", genoType, "format."))
...
# split markers into a list of multiple chunks each of which includes no more than nMarkers_output markers
