...
| `--outlier-iqr-threshold`       | 1.5                                  | IQR multiplier used for residual outlier detection in SPAGRM / SAGELD / SPAmix / SPAmixPlus / SPAmixLocalPlus / WtCoxG / LEAF / SPAsqr.                              | Lower values flag more outliers and slow the per-family SPA tail; defaults are tuned for inverse-normal-transformed residuals.                                          |
...
2. Build `SPAGRMClass` null model:
   ...
   - IQR-based outlier detection (ratio = `--outlier-iqr-threshold`).
